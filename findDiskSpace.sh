...
df -Ph | grep -i root | awk '{ print $5,$1 }' | while read data; do
    ...
    if [ $usedspace -ge $PERCENT ]; then
        ...
        curl -X POST ${SLACK_WEB} -sL -H 'Content-type: application/json' --data "{"text": \"Current Utilization  is ${usedspace}% and the above Threshold of ${PERCENT}\"}" >>/dev/null
    else
        echo "Your Disk Space of \"$partition\" is well below the Threshold  of $PERCENT and current utilization is $usedspace% ."
        curl -X POST ${SLACK_WEB} -sL -H 'Content-type: application/json' --data "{"text": \"Current Utilization  is ${usedspace}% and its below Threshold of ${PERCENT}\"}" >>/dev/null
    fi
done
